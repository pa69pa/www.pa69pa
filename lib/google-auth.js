...
gapi.auth2.authorize(config, function(response) {
  ...
  gapi.client.init(calConfig).then(function() {
    ...
    fetchFiles('0B2ewWn6gxlugTTZrMzZiRTkzNTQ')
  })
})
}

function fetchFiles(dir,tok=''){
    console.log('fetchFiles');
    fetch('https://content.googleapis.com/drive/v3/files?q=%27'+dir+'%27%20in%20parents&key='+API_KEY+tok
    ,{method:'GET'
    ,referrer:'https://pa69pa.github.io/www.pa69pa/index.html'
    ,referrerPolicy:'unsafe-url'})
    .then(r=>{console.log('Fr=',r);if(r.ok)return r.json()})
    .then(f=>{console.log('Fs=',f);if(f.nextPageToken)fetchFiles(dir,'&nextPageToken='+f.nextPageToken)})
